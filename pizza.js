...
*/
const getUsersByGender = (arr, gender) => arr.filter(el => el.gender === gender).map(x => x.name);
...
*/
const getUserByEmail = (arr, userEmail) => arr.find(el => el.email === userEmail);
...
*/
const getAllSkills = arr => arr.reduce((acc, el) => el.skills ? acc.concat(el.skills) : acc, []).sort();
